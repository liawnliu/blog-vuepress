...
两种方式的差异和应用会在最后一小节[“两种方式的区别和应用场景”](#两种方式的区别和应用场景)里讲述。
...
script脚本除了生命周期函数以外，还有一些鼠标点击、键盘和碰撞等事件，当然也不止这些事件，可以去[laya.components.Script](http://layaair2.ldc2.layabox.com/api2/Chinese/index.html?category=Core&class=laya.components.Script)这个API里查看事件。  
...
之前在[2.IDE的使用#项目设置](2.IDE的使用.md#项目设置)里提到过“场景的发布模式”，这里简单做一个总结：
