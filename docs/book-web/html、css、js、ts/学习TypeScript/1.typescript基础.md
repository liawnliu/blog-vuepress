...
函数Function是基于Object的，它主要用于描述一类行为，执行一些特定的代码，可以参考查看js里的[function类型](/book-web/html、css、js、ts/学习JavaScript/3.引用类型.md#五、function类型)。
...
我们知道js的数组是可以存不同类型的值，数组的长度也是动态改变的，详情可以查看js的[array类型](/book-web/html、css、js、ts/学习JavaScript/3.引用类型.md#二、array类型)。
...
与react有关的可以看一下[学习react的准备工作](/book-web/web前端js框架/学习React/)
