...
在`src`下新建文件夹`controller`，用于存放某些路由对应的数据处理的Js。
...
因为我们使用的原始Node开发接口，这个POST请求的参数接收是`req.on('data')`和`req.on('end')`，它们是一个异步操作，需要使用`Promise`来包装它们。

可以看到下面的代码演示，我们在`getPostData`函数里使用`new Promise`对`req.on('data')`和`req.on('end')`进行了包装，只有等这个`Promise`在resolve()后才能获取到postData并存储在`req.body`里了。这里要注意的是`req.body = await getPostData(req)`后面的代码，是在`await`这里的异步操作执行完之后才能执行的。
